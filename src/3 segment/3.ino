 ...
 if(CAN_MSGAVAIL == CAN.checkReceive())          
    {
        ...
        //funkcje dla wszystkich, przychodzące od pierwszego segmentu
        if(canId == 51){
          for(int i = 0; i<len; i++){ 
              Serial.print(buf[i]);
              Serial.print("\t");
              ...
              if(buf[3]==6){sparkles();}
              if(buf[3]==8){hue=buf[2]; fireEffect();}
          Serial.println();
        }
        //funkcje przychodzące od poprzedniego segmentu, wykonywanie funkcji przejścia
        if(canId == 52){
          for(int i = 0; i<len; i++){
              Serial.print(buf[i]);
              Serial.print("\t");
...
void colorSwitch(int hueFromFunction){
    int brightness = 220;
    FastLED.setBrightness(brightness);
    fill_solid( leds, NUM_LEDS, CHSV(hueFromFunction,255,brightness));
    FastLED.show();
}
...
//    fadeToBlackBy(leds,NUM_LEDS,20);
  ...
  unsigned char message[4] = {7, 0, ghue, 7};
